...
interface StatsDao {
    ...
    @Query("SELECT strftime(\"%w\", time, 'unixepoch', 'localtime') AS day, " +
    ...
    @Query("SELECT strftime(\"%H\", time, 'unixepoch', 'localtime') AS hour, count(*) AS count " +
    ...
    @Query("SELECT name, artist_names, image, count(*) as count FROM Scrobble " +
